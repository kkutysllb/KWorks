...
function buildInjection(
  ...
  budgetBytes: number,
  context: { workspace?: string; loadedSkills: readonly LoadedSkillPlugin[] }
...
function legacySkillPathMappings(skills: readonly Pick<LoadedSkillPlugin, 'id' | 'root'>[]): Array<{ from: string; to: string }> {
...
function runtimePathMappingNote(skill: Pick<LoadedSkillPlugin, 'id' | 'root'>, workspace?: string): string {
  const lines = [
