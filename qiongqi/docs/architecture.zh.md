...
Qiongqi 会替换 kk_KWorks 的 `coding_core`，不会借用 LangGraph/LangChain/Python core 作为内部编排合同。kk_KWorks 只作为产品运行面经验来源：其中可复用的安全、预算、路径、记忆、终态治理模式已按 Qiongqi 原生包边界重实现。
...
### 4.4 OrchestrationMode：Kernel v3 与 classic/evented 兼容模式

生产 runtime 现在默认选择 `kernel_v3`。`classic` 和 `evented`/`evented_v2` 仍可通过 `QiongqiServeRuntimeOptions.orchestrationMode` 显式选择，作为兼容模式保留。

| 模式 | 编排器 | 崩溃恢复 | 适用场景 |
|------|-------|---------|---------|
| `kernel_v3` | 带检查点和副作用幂等的持久化 Kernel loop | 有 | 生产默认模式 |
| `classic` | `TurnOrchestrator`（显式 step 推进）| 无 | 显式兼容回退 |
