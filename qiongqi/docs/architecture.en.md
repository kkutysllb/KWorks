...
Qiongqi replaces kk_KWorks `coding_core`; it does not borrow LangGraph, LangChain, or Python core contracts as internal orchestration. kk_KWorks is used only as a product/runtime-governance reference, and the reusable safety, budgeting, path, memory, and terminal-state patterns are reimplemented inside Qiongqi-native package boundaries.
