...
describe('MCP tool provider', () => {
  ...
  it('treats default and relative workspaces as the runtime working directory for trust checks', () => {
    const server = {
      enabled: true,
      transport: 'stdio',
      command: 'node',
    ...
    expect(isMcpServerTrusted(server, 'default')).toBe(true)
    expect(isMcpServerTrusted(server, '.')).toBe(true)
  })
