...
describe('ModelStepRunner', () => {
  ...
  it('persists reasoning signatures on completed reasoning items', async () => {
    const h = makeHarness(
      makeFakeModel([
        { kind: 'assistant_reasoning_delta', text: 'thinking' },
    ...
    const reasoning = (await h.sessionStore.loadItems(h.threadId))
      .find((item) => item.kind === 'assistant_reasoning')
