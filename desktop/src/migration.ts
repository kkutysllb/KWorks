 ...
 * - memory:        <repo>/.kkworks/memory.json
 * - agents:        <repo>/.kkworks/agents/
 *
 * The desktop home (`~/.kworks-workspace`) uses a flat layout where all
...
export function getWebMemoryPath(repoRoot: string): string {
  return join(repoRoot, ".kkworks", "memory.json");
}
export function getWebAgentsPath(repoRoot: string): string {
  return join(repoRoot, ".kkworks", "agents");
}
 ...
 * `.kkworks/memory.json`) to decide whether a given directory is a
 ...
 * from the scattered web-side layout (skills/custom, .env, .kkworks, …).
 */
...
function migrateAgents(
  ...
  // Web source: <repo>/.kkworks/agents/  →  Desktop target: <home>/agents/
...
function migrateMemory(
  ...
  // Web source: <repo>/.kkworks/memory.json  →  Desktop target: <home>/memory.json
...
async function migrateExtensions(
  ...
  // Web source: <repo>/extensions_config.json → current user's QiongQi MCP/skill config.
  ...
  if (!context.authToken) {
    return {
      ...
      skipped: 0,
      merged: 0,
      error: "缺少登录凭证，无法导入 MCP/技能配置",
    };
  }
  const baseUrl = context.gatewayBaseUrl ?? "http://127.0.0.1:19987";
  const currentResponse = await fetch(`${baseUrl}/api/mcp/config`, {
  ...
  if (!currentResponse.ok) {
    return {
      ...
      skipped: 0,
      merged: 0,
      error: `读取当前 MCP/技能配置失败 (${currentResponse.status})`,
    };
  }
  const dst = (await currentResponse.json()) as {
    mcpServers?: Record<string, unknown>;
    mcp_servers?: Record<string, unknown>;
    skills?: Record<string, unknown>;
  };
  const mergedMcp = { ...(dst.mcpServers ?? dst.mcp_servers ?? {}), ...(src.mcpServers ?? {}) };
  const mergedSkills = { ...(dst.skills ?? {}), ...(src.skills ?? {}) };
  const merged = {
    mcpServers: mergedMcp,
    skills: mergedSkills,
  };
  const saveResponse = await fetch(`${baseUrl}/api/mcp/config`, {
    method: "PUT",
    headers: {
  ...
  if (!saveResponse.ok) {
    const detail = (await saveResponse.json().catch(() => ({}))) as { detail?: string };
    return {
      category: "extensions",
      copied: 0,
      skipped: 0,
      merged: 0,
      error: detail.detail ?? `保存 MCP/技能配置失败 (${saveResponse.status})`,
    };
  }
  const mergedCount =
...
export async function executeMigration(
  ...
  if (options.extensions) {
    try {
      const result = await migrateExtensions(sourceRepoRoot, context);
