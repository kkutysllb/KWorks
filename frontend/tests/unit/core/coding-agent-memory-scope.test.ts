...
describe("coding agent memory scope", () => {
  ...
  test("agent panel binds the input workspace root to the opened project", () => {
    const agentPanel = readFileSync(
      resolve(repoRoot, "src/components/workspace/coding/agent-panel.tsx"),
      "utf8",
    );

    expect(agentPanel).toContain("syncCodingProjectContext");
    expect(agentPanel).toContain('workModeId: "coding"');
    expect(agentPanel).toContain("workspaceRoot: project.path");
    expect(agentPanel).toContain("settings.context.workspaceRoot === project.path");
  });
});
