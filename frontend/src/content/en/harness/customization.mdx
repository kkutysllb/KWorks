---
title: Customization
description: KWorks's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
---
...
<Callout type="info" emoji="🔧">
  KWorks is designed to be adapted. You can extend agent behavior by writing
...
KWorks's pluggable architecture means most parts of the system can be replaced or extended without forking the core. This page maps the extension points and explains how to use each one.
...
from kkworks.agents.thread_state import ThreadState
...
```python
from kkworks.sandbox.sandbox_provider import SandboxProvider
from kkworks.sandbox.sandbox import Sandbox
...
```python
from kkworks.agents.memory.storage import MemoryStorage
...
For custom backends, implement the LangGraph `BaseCheckpointSaver` interface and configure it programmatically when initializing the `KWorksHarnessClient`.
...
guardrails:
  enabled: true
  provider:
    use: kkworks.guardrails.builtin:AllowlistProvider
    config:
...
<Cards num={2}>
  <Cards.Card
    ...
    href="/en/docs/harness/integration-guide"
  />
