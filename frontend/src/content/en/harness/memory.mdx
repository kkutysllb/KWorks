---
title: Memory
description: Memory is a runtime feature of the KWorks Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets KWorks carry useful information across sessions. The agent
...
Memory is a runtime feature of the KWorks Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  # Default: {base_dir}/memory.json  (resolves to backend/.kkworks/memory.json)
  ...
  storage_class: kkworks.agents.memory.storage.FileMemoryStorage
...
KWorks supports two levels of memory:
...
- Base directory: `backend/.kkworks/`
- Global memory: `backend/.kkworks/memory.json`
- Per-agent memory: `backend/.kkworks/agents/{agent_name}/memory.json`
