...
description: "This guide covers all supported deployment methods for KWorks App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for KWorks App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run KWorks. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your kkworks repo root
export KWorks_ROOT=/path/to/kkworks
...
# .env
OPENAI_API_KEY=sk-...
KWorks_ROOT=/absolute/path/to/kkworks
...
Thread data is stored in `backend/.kkworks/threads/`. In Docker deployments, this directory is bind-mounted into the langgraph container.

To avoid data loss when containers are recreated:

1. Set `KWorks_ROOT` to the absolute repo root path (or a stable host path).
...
USERDATA_PVC_NAME=kkworks-userdata-pvc
SKILLS_PVC_NAME=kkworks-skills-pvc
```
...
KWorks App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
