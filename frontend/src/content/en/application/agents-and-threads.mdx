...
description: KWorks App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
KWorks App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
<Callout type="info">
  ...
  embedded <code>KWorksHarnessClient</code> (used in direct Python integrations), not
  to the LangGraph Server deployment used by KWorks App.
</Callout>
...
```
backend/.kkworks/threads/{thread_id}/user-data/
```

This directory is mounted into the sandbox as `/mnt/user-data/` for the agent to read and write.

In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `KWorks_ROOT` to the absolute path of your kkworks repository root to ensure the correct host path is used.

<Cards num={2}>
  <Cards.Card
    title="Workspace Usage"
    href="/en/docs/application/workspace-usage"
  />
  <Cards.Card
