...
description: This page covers day-to-day operational tasks and solutions to common problems when running KWorks App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running KWorks App.
...
| File                 | Service                              |
| -------------------- | ------------------------------------ |
| `logs/langgraph.log` | LangGraph / KWorks Harness runtime |
...
When you pull a new version of KWorks, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
### MiniMax-M3 leaks tool protocol text

Symptoms: when using MiniMax-M3, the main chat shows text such as `<action>run</action>`, `<timeout>`, `tool call`, `[<invoke name="bash">]`, or long tool execution payloads instead of normal tool-call results. The task can also stop after this content appears.

Cause: the official MiniMax-M3 OpenAI-compatible API should return standard `tool_calls`, and requests should enable `reasoning_split` to separate reasoning content. KWorks enables `reasoning_split` automatically and includes a fallback parser for the rare responses that still stream model-native tool protocol inside `delta.content`, converting them into internal tool calls instead of showing them in the main chat.

If you are actually self-hosting MiniMax-M3 with local vLLM, also start vLLM with the official MiniMax-M3 parser flags:
...
Note: the official MiniMax-M3 API does not use these vLLM server flags. They only apply when the `baseUrl` points to a local vLLM server that is actually hosting MiniMax-M3; a local vLLM deployment of DeepSeek-v4-flash does not use this MiniMax-M3 parser rule.
...
3. For K8s Provisioner: check that the provisioner service is healthy (`curl http://localhost:9194/health`) and that the K8s cluster is reachable.
...
```
Connection refused: http://provisioner:9194
```
...
- Check that the storage path is writable: `ls -la backend/.kkworks/`.
- Look for memory update errors in `logs/langgraph.log` (search for "memory").

## Data backup

Thread data and memory are stored under `backend/.kkworks/`:
...
In Docker deployments, the bind-mounted host path (`$KWorks_ROOT/backend/.kkworks/`) is the source of truth — back up the host path.
...
<Cards num={2}>
  <Cards.Card
    ...
    href="/en/docs/application/deployment-guide"
  />
