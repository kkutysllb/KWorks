---
title: Configuration
description: KWorks App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
KWorks App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
models:
  - name: deepseek-v3
    use: kkworks.models.patched_deepseek:PatchedChatDeepSeek
...
Install Ollama provider: `cd backend && uv add 'kkworks-harness[ollama]'`
...
```yaml
sandbox:
  use: kkworks.community.aio_sandbox:AioSandboxProvider
  image: enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest
  replicas: 3
  idle_timeout: 600
```

Install: `cd backend && uv add 'kkworks-harness[aio-sandbox]'`
...
tools:
  ...
  - use: kkworks.community.ddg_search.tools:web_search_tool # default, no key required
  # - use: kkworks.community.tavily.tools:web_search_tool
  #   api_key: $TAVILY_API_KEY

  # Web fetch (choose one)
  - use: kkworks.community.jina_ai.tools:web_fetch_tool

  # Image search
  - use: kkworks.community.image_search.tools:image_search_tool

  # File operations
  - use: kkworks.sandbox.tools:ls_tool
  - use: kkworks.sandbox.tools:read_file_tool
  - use: kkworks.sandbox.tools:glob_tool
  - use: kkworks.sandbox.tools:grep_tool
  - use: kkworks.sandbox.tools:write_file_tool
  - use: kkworks.sandbox.tools:str_replace_tool
  - use: kkworks.sandbox.tools:bash_tool
```

### Thread state persistence (checkpointer)

By default, KWorks uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.kkworks/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/kkworks
```
...
<Cards num={2}>
  ...
  <Cards.Card
    ...
    href="/en/docs/harness/configuration"
  />
</Cards>
