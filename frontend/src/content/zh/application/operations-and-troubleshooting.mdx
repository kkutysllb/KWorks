---
title: 运维与排障
description: 本页面涵盖运行 KWorks 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 KWorks 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

KWorks 应用在 `logs/` 目录中写入每个服务的日志：
...
**症状**：使用 MiniMax-M3 时，主界面出现 `<action>run</action>`、`<timeout>`、`tool call`、`[<invoke name="bash">]` 或大段工具执行文本，而不是正常工具调用结果；任务也可能在这类内容后停止。

**原因**：MiniMax-M3 官方 OpenAI 兼容接口应返回标准 `tool_calls`，并建议请求中启用 `reasoning_split` 来拆分思考内容。KWorks 会自动启用 `reasoning_split`，并对少数把模型原生工具协议混入 `delta.content` 的响应做兜底解析，转成内部工具调用而不是显示到主界面。

如果你实际使用的是本地 vLLM 自部署 MiniMax-M3，还需要在启动 vLLM 时按官方 recipe 带上 MiniMax-M3 parser 参数：
...
注意：官方 MiniMax-M3 API 不需要这些 vLLM 启动参数。只有当 `baseUrl` 指向本地 vLLM 且模型确实是 MiniMax-M3 时，才需要检查 parser 参数；本地 vLLM 部署 DeepSeek-v4-flash 不适用这条 MiniMax-M3 parser 规则。
...
```bash
# 检查线程用户数据目录是否存在且可写
ls -la backend/.kkworks/threads/
```

**解决**：

1. 确保 `backend/.kkworks/` 对运行 KWorks 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`KWorks_ROOT` 设置为绝对路径）。
...
- **线程数据**：`backend/.kkworks/threads/` — 每个线程的上传文件、输出和工作区文件
- **检查点**：取决于检查点器配置（SQLite：`backend/.kkworks/checkpoints.db`，Redis：外部存储）
- **记忆**：`backend/.kkworks/memory.json`（以及 `agents/*/memory.json`）
...
<Cards num={2}>
  <Cards.Card title="部署指南" href="/zh/docs/application/deployment-guide" />
  <Cards.Card title="配置" href="/zh/docs/application/configuration" />
</Cards>
