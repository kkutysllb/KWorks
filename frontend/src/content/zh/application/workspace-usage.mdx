---
title: 工作区使用
description: KWorks 工作区是一个基于浏览器的对话界面，你可以在其中向 Agent 发送消息、上传文件、查看中间步骤，以及下载生成的产出物。
---
...
KWorks 工作区是一个基于浏览器的对话界面，你可以在其中向 Agent 发送消息、上传文件、查看中间步骤，以及下载生成的产出物。

## 新建对话

1. 打开 [http://localhost:9191](http://localhost:9191)。
...
产出物会持久保存在线程的用户数据目录中（`.kkworks/threads/{thread_id}/user-data/outputs/`）。
...
<Cards num={2}>
  <Cards.Card
    title="Agent 与线程"
    href="/zh/docs/application/agents-and-threads"
  />
  <Cards.Card
    title="运维与排障"
