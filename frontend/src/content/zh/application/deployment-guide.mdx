---
title: 部署指南
description: 本指南涵盖 KWorks 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。
---
...
本指南涵盖 KWorks 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。

## 本地开发部署

本地工作流是运行 KWorks 最快的方式，所有服务作为原生进程在你的机器上运行。
...
| 服务        | 端口 | 描述                    |
| ----------- | ---- | ----------------------- |
| Gateway API（含内嵌运行时） | 9193 | FastAPI 后端 + Harness |
| 前端                       | 9192 | Next.js 界面           |
| nginx                      | 9191 | 统一反向代理           |
...
```bash
# 设置 kk_KWorks 仓库根目录的绝对路径
export KWorks_ROOT=/path/to/kk_KWorks
...
# .env
OPENAI_API_KEY=sk-...
KWorks_ROOT=/absolute/path/to/kk_KWorks
...
线程数据存储在 `backend/.kkworks/threads/`。在 Docker 部署中，此目录被绑定挂载到 langgraph 容器中。

为避免容器重建时数据丢失：

1. 将 `KWorks_ROOT` 设置为仓库根目录的绝对路径。
...
```bash
# 在 .env 或 compose 环境中
USERDATA_PVC_NAME=kk-KWorks-userdata-pvc
SKILLS_PVC_NAME=kk-KWorks-skills-pvc
```
...
KWorks 应用使用 [Better Auth](https://www.better-auth.com/) 进行会话管理。在生产环境中：
...
<Cards num={2}>
  ...
  <Cards.Card
    title="运维与排障"
    href="/zh/docs/application/operations-and-troubleshooting"
  />
</Cards>
