---
title: Agent 与线程
description: 了解 KWorks 中 Agent 与线程的关系，以及如何管理自定义 Agent 和对话线程。
---
...
KWorks 允许你创建多个具有不同专业领域的自定义 Agent。每个 Agent 使用 KWorks Harness 相同的 Lead Agent 运行时，但具有不同的：
...
```bash
curl -X POST http://localhost:9193/api/agents \
...
KWorks 在每次 Agent 轮次后自动保存线程状态（如果配置了检查点器）。这允许：
...
每个线程在 `.kkworks/threads/{thread_id}/` 下有一个专用目录：

```
.kkworks/threads/{thread_id}/
  user-data/
    uploads/     ← 用户上传的文件
...
<Cards num={2}>
  <Cards.Card title="工作区使用" href="/zh/docs/application/workspace-usage" />
  <Cards.Card
    title="运维与排障"
