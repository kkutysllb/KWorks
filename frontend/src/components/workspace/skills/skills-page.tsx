...
import type { WorkModeSkill } from "@/core/skills/type";
import {
...
const EMPTY_WORK_MODE_SKILLS: WorkModeSkill[] = [];
...
function SkillCard({
  ...
  return (
    <div
      className={cn(
        "group bg-card flex items-center gap-4 rounded-lg border px-4 py-3 transition-all duration-200",
...
export function SkillsPage() {
  ...
  const skills = selectedSkillView?.skills ?? EMPTY_WORK_MODE_SKILLS;
  ...
  const handleSkillEnabledChange = (skill: WorkModeSkill, enabled: boolean) => {
    ...
    mutation({ workModeId: activeWorkModeId, skillId: skill.id ?? skill.name });
  };
        ...
        <div className="relative px-6 pt-7 pb-5">
            ...
            <div className="flex items-center gap-3">
              {skills.length > 0 && (
                <div className="text-muted-foreground hidden items-center gap-2 text-xs sm:flex">
                  ...
                  {viewCountLabel} {skills.length} 个技能
                </div>
              )}
              <WorkModeDialog
                workModes={workModes}
              ...
              <Button
                ...
                className="bg-gradient-to-r from-violet-500 to-cyan-500 text-white shadow-md shadow-violet-500/25 transition-all duration-200 hover:from-violet-600 hover:to-cyan-600 hover:shadow-lg hover:shadow-violet-500/30"
              >
                <SparklesIcon className="size-4" />
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative max-w-sm flex-1">
              ...
              <Input
                className="border-muted-foreground/20 bg-muted/50 h-10 rounded-xl pl-10 focus-visible:ring-violet-500/30"
