...
export function ImportWizardDialog({
  ...
  useEffect(() => {
    if (!open) return;
    void (async () => {
      if (!window.kworksDesktop?.detectMigrationSources) return;
      try {
        const sources = await window.kworksDesktop.detectMigrationSources();
  ...
  const pickDirectory = useCallback(async () => {
    if (!window.kworksDesktop?.pickDirectory) return;
    const picked = await window.kworksDesktop.pickDirectory({
  ...
  const doScan = useCallback(async (path: string) => {
    if (!window.kworksDesktop?.scanMigrationSource || !path) return;
    setScanning(true);
    try {
      const res = (await window.kworksDesktop.scanMigrationSource(
        path,
      ));
      setScan(res);
      return res;
  ...
  const handleExecute = useCallback(async () => {
    ...
    try {
      ...
      const res = (await window.kworksDesktop?.executeMigration({
        sourceRepoRoot,
        options,
        authToken: getDesktopSessionToken() ?? undefined,
      }))!;
      clearInterval(timer);
      setProgress(100);
      setResult(res);
