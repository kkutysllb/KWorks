 ...
 * page the user lands on. Inactive on web (no `window.kworksDesktop`).
 */
export function MigrationPromptListener() {
  ...
  useEffect(() => {
    if (!isDesktop() || !window.kworksDesktop?.onMigrationAvailable) return;
    const unsubscribe = window.kworksDesktop.onMigrationAvailable((next) => {
