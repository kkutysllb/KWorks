 ...
 * previously rendered messages instantly while the `useQiongqiStream` hook
 * silently reconnects the SSE stream in the background.
 *
 * Without this, the user sees a flash of:
 *   1. empty message list (the stream initialises with `messages: []`)
 ...
 * running on the qiongqi runtime.
 *
