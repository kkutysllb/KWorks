import type { AIMessage, Message } from "../threads/qiongqi-types";
...
export function extractContentFromMessage(message: Message) {
  const sanitizeForDisplay = (content: string) => stripInternalContent(content);
...
const AGENT_ARTIFACT_HEADER_RE =
  /^(?:#\s*)?(?:SESSION\s+INTENT|SUMMARY|ARTIFACTS?|NEXT\s+STEPS)(?:[\s\n]|$)/i;
...
export function isHiddenFromUIMessage(message: Message) {
  ...
  if (message.type !== "tool" && !("tool_calls" in message && message.tool_calls?.length)) {
    if (
      ...
      AGENT_ARTIFACT_HEADER_RE.test(message.content.trim())
    ) {
      return true;
    }
