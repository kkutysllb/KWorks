...
description: >-
  ...
  task history must be isolated from other KWorks tasks, global memory, or
...
Prevent Coding Agent state from leaking into or depending on unrelated KWorks
task memory.

## Workflow

1. Treat Coding memory as scoped by thread id and project root.
2. Store durable Coding runtime state under the Qiongqi/KWorks Coding session
   store.
