...
Tool call sequence:
1. activate_skill("strategy-generate") → 获得工作流指引
2. write("config.json") → 配置标的/日期/参数
   ```json
   ...
   ```
3. write("code/signal_engine.py") → 双均线策略代码
4. bash("python -c \"import ast; ast.parse(open('code/signal_engine.py').read()); print('OK')\"") → AST 语法检查
5. backtest(run_dir=...) → 执行回测（引擎内置）
6. read("artifacts/metrics.csv") → 查看结果，按评审标准判断
7. (如需修复) edit("code/signal_engine.py", ...) → backtest → read
...
2. write("config.json") → 配置
   ...
   ```
3. write("code/signal_engine.py") → RSI 策略代码
4. bash("python -c \"import ast; ast.parse(open('code/signal_engine.py').read()); print('OK')\"") → AST 检查
5. backtest(run_dir=...) → 执行回测（引擎内置）
6. read("artifacts/metrics.csv") → 查看结果
7. (如需修复) edit → backtest → read
...
2. write("config.json") → 配置
   ...
   ```
3. write("code/signal_engine.py") → 趋势策略代码
4. bash("python -c \"import ast; ast.parse(open('code/signal_engine.py').read()); print('OK')\"") → AST 检查
5. backtest(run_dir=...) → 执行回测（引擎内置）
6. read("artifacts/metrics.csv") → 查看结果
7. (如需修复) edit → backtest → read
