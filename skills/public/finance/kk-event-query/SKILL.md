...
接收用户的自然语言查询请求，分析用户意图，识别事件数据相关查询类型：
- 业绩预告查询
- 增发配股/股权质押查询
- 限售解禁/机构调研查询
- 监管函/重大事件查询
...
| `X-Claw-Skill-Id` | `hithink-event-query`（与 skill name 一致） |
...
headers = {
    ...
    "X-Claw-Skill-Id": "hithink-event-query",
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含股票代码、股票简称、事件类型、公告日期等字段 |
...
{
  "datas": [
    {"股票代码": "300033.SZ", "股票简称": "同花顺", "事件类型": "业绩预告", "公告日期": "20240115"}
  ],
  "code_count": 156,
  "chunks_info": {
    "query": "同花顺业绩预告",
    "parsed_conditions": ["同花顺", "业绩预告"]
  }
}
```
...
python3 scripts/cli.py --query "同花顺业绩预告" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "同花顺业绩预告" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-event-query" \
...
} -Body '{"query":"同花顺业绩预告","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
