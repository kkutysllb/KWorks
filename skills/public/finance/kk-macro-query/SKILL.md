...
capabilities:
  ...
  - id: cpi-ppi-query
    description: "CPI/PPI数据查询：居民消费价格指数与工业生产者出厂价格指数"
  - id: interest-rate
    description: "利率查询：LPR、MLF、逆回购等政策利率"
  - id: exchange-rate
    description: "汇率查询：人民币兑美元、欧元、日元等主要货币汇率"
  - id: social-financing
    description: "社融数据查询：社会融资规模、M2、新增贷款等"
  - id: economic-indicators
    description: "经济运行指标查询：PMI、工业增加值、消费、投资、进出口等"
...
接收用户的自然语言查询请求，分析用户意图，识别宏观数据相关查询类型：
- 国民经济核算指标查询
- 金融指标查询
- 经济运行指标查询
...
| `X-Claw-Skill-Id` | `hithink-macro-query`（与 skill name 一致） |
...
headers = {
    ...
    "X-Claw-Skill-Id": "hithink-macro-query",
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含指标名称、数值、时间等字段 |
...
```json
{
  ...
  "chunks_info": {
    "query": "2024年中国GDP",
    "parsed_conditions": ["2024年", "中国", "GDP"]
  }
}
```
...
python3 scripts/cli.py --query "2024年中国GDP" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "2024年中国GDP" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-macro-query" \
...
} -Body '{"query":"2024年中国GDP","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
