...
capabilities:
  ...
  - id: major-customers
    description: "主要客户信息查询：客户名称、销售占比等"
  - id: supplier-info
    description: "供应商信息查询：供应商名称、采购占比等"
  - id: subsidiary-investment
    description: "参控股公司与股权投资查询：子公司、参股公司信息"
  - id: major-contracts
    description: "重大合同信息查询"
...
接收用户的自然语言查询请求，分析用户意图，识别经营数据相关查询类型：
- 主营业务构成查询
- 主要客户/供应商查询
- 参控股公司/股权投资查询
- 重大合同查询
...
| `X-Claw-Skill-Id` | `hithink-business-query`（与 skill name 一致） |
...
| 参数名 | 类型 | 说明 |
|--------|------|------|
| datas | ARRAY | 金融数据列表，对象数组，每个对象包含股票代码、股票简称、业务类型、收入占比等字段 |
...
python3 scripts/cli.py --query "同花顺主营业务构成" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "同花顺主营业务构成" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-business-query" \
...
} -Body '{"query":"同花顺主营业务构成","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
